  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-28 pt-4 px-4">
      <div className="max-w-md mx-auto space-y-5">
        ...
        <div className="bg-gradient-to-br from-[#0d5c91] to-sky-900 text-white p-5 shadow-lg space-y-4 relative overflow-hidden">
          <div className="absolute -right-6 -bottom-6 w-32 h-32 bg-white/5 blur-xl pointer-events-none" />
          
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              {profile?.avatar ? (
                <img src={profile.avatar} alt={profile.name} className="w-12 h-12 object-cover border-2 border-white/30" />
              ) : (
                <div className="w-12 h-12 bg-white/10 text-white flex items-center justify-center font-bold text-lg border-2 border-white/30">
          ...
          <div className="bg-white/15 backdrop-blur-md p-3.5 border border-white/15 grid grid-cols-3 gap-2 text-center">
            <div>
        ...
        <div className="bg-emerald-50 border border-emerald-200/80 p-4 shadow-xs space-y-2">
        ...
        <div className="space-y-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-white border border-slate-200 pl-10 pr-4 py-2.5 text-xs sm:text-sm font-medium text-slate-800 focus:outline-[#0d5c91]"
            />
          </div>

          <div className="flex bg-slate-200/70 p-1 text-xs font-bold text-slate-600">
            <button
              onClick={() => setActiveTab('semua')}
              className={`flex-1 py-2 transition cursor-pointer ${activeTab === 'semua' ? 'bg-white text-slate-900 shadow-xs' : 'hover:text-slate-900'}`}
            >
              Semua ({donations.length})
            </button>
            <button
              onClick={() => setActiveTab('pending')}
              className={`flex-1 py-2 transition cursor-pointer ${activeTab === 'pending' ? 'bg-white text-amber-700 shadow-xs' : 'hover:text-slate-900'}`}
            >
              Pending
            </button>
            <button
              onClick={() => setActiveTab('sukses')}
              className={`flex-1 py-2 transition cursor-pointer ${activeTab === 'sukses' ? 'bg-white text-emerald-700 shadow-xs' : 'hover:text-slate-900'}`}
            >
              Berhasil
            </button>
          </div>
          ...
          <div className="flex items-center justify-between gap-2 text-xs">
            <select
              ...
              className="bg-white border border-slate-200 px-3 py-2 font-semibold text-slate-700 focus:outline-none"
            >
        ...
        {filteredDonations.length === 0 ? (
          <div className="text-center py-16 px-4 bg-white border border-slate-200 shadow-xs space-y-4">
            <div className="w-12 h-12 bg-sky-50 text-[#0d5c91] flex items-center justify-center mx-auto">
            ...
            <Link 
              href="/" 
              className="inline-block bg-[#0d5c91] hover:bg-sky-900 text-white px-6 py-3 font-bold text-xs uppercase tracking-wider transition shadow-sm"
            >
              Mulai Berdonasi
            </Link>
          </div>
          ...
          <div className="space-y-3">
            {filteredDonations.map((d: any) => {
              ...
              return (
                <div key={d.id} className="bg-white p-4 border border-slate-200/80 shadow-xs space-y-3 hover:border-sky-300 transition">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <span className="text-[10px] font-bold text-sky-700 uppercase tracking-wider bg-sky-50 px-2 py-0.5 border border-sky-100">
                  ...
                  <div className="flex items-center justify-between text-[11px] text-slate-400 pt-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {isPending && d.payment_url && (
                        <a
                          href={d.payment_url}
                          className="bg-[#e91e63] hover:bg-pink-700 text-white font-bold px-3 py-1.5 flex items-center gap-1 transition shadow-xs text-xs"
                        >
                      ...
                      {!isPending && (
                        <Link
                          ...
                          className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold px-3 py-1.5 transition text-xs"
                        >
                          Donasi Lagi
                        </Link>
                      )}
      ...
      {selectedDonation && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md p-5 space-y-4 shadow-2xl border border-slate-200 text-left max-h-[90vh] overflow-y-auto">
            ...
            <div className="pt-2 flex gap-2">
              <button
                ...
                className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold py-2.5 transition text-xs flex items-center justify-center gap-1.5 cursor-pointer"
              >
              ...
              <button
                ...
                className="flex-1 bg-[#0d5c91] hover:bg-sky-900 text-white font-bold py-2.5 transition text-xs flex items-center justify-center gap-1.5 cursor-pointer"
              >
                Bagikan Kebaikan ❤️
              </button>
            </div>
