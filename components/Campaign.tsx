        ...
        <section className="bg-white rounded-2xl p-4 sm:p-5 border border-gray-200/90 shadow-sm space-y-3.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 pt-1">
            {mendesak.map((item) => {
              ...
              const percentage = Math.min(Math.round((item.collectedRaw / target) * 100), 100);
              return (
                <Link key={item.id} href={`/campaign/${item.slug}`} className="group border border-gray-200/90 rounded-2xl overflow-hidden p-3 bg-gray-50/60 space-y-2.5 block hover:shadow-md transition">
                  <div className="aspect-[16/10] bg-gray-200 rounded-xl overflow-hidden relative">
                    <img src={item.image} alt={item.title} className="w-full h-full object-cover group-hover:scale-105 transition duration-300" />
                    {item.daysLeft && (
                      <span className="absolute top-2 left-2 bg-[#0d5c91] text-white text-[10px] font-bold px-2 py-0.5 rounded-lg shadow">
        ...
        <section className="bg-white rounded-2xl p-4 sm:p-5 border border-gray-200/90 shadow-sm space-y-3.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 pt-1">
            {unggulan.map((item) => {
              ...
              return (
                <Link key={item.id} href={`/campaign/${item.slug}`} className="group border border-gray-200/90 rounded-2xl overflow-hidden p-3 bg-gray-50/60 space-y-2.5 block hover:shadow-md transition">
                  <div className="aspect-[16/10] bg-gray-200 rounded-xl overflow-hidden">
                    <img src={item.image} alt={item.title} className="w-full h-full object-cover group-hover:scale-105 transition duration-300" />
                  </div>
                  <h3 className="text-xs sm:text-sm font-bold text-slate-800 line-clamp-2 leading-snug group-hover:text-[#0d5c91] transition-colors">{item.title}</h3>
                  <div className="space-y-1">
